...
from table_detect.table_detect_document import TableDetectDocument
import pickle
...
class PrepareDataset:
    ...
    def load(self):
        ...
        for i in os.listdir(self.path):
            ...
            spatial_features = model.get_feature_map(image).astype(np.float64)
            ...
            self.dump_doc(all_tokens, all_tokens_rects, image, spatial_features, document_dump_path)
            ii += 1
            print("Loaded %d" % ii)

        print("Data loaded")

    def dump_doc(self, all_tokens, all_tokens_rects, image, spatial_features, file_name):
        N = len(all_tokens)
        height, width = np.shape(image)
        classes = np.zeros(N)
        ...
        document = TableDetectDocument(embeddings_matrix, rect_matrix, neighbor_distance_matrix, neighbor_graph, classes, conv_features)
